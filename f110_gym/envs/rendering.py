...
class EnvRenderer(pyglet.window.Window):
    ...
    def update_map(self, map_name, map_ext):
        ...
        # load map metadata
        map_path = "maps/" + map_name + "/" + map_name + "_map"  
        ...
        map_img_path = "maps/" + map_img_name.split("_")[0] + "/" + map_img_name 
        ...
        if self.map_points.shape[0] > 0:
            min_x = np.min(self.map_points[:, 0])
            max_x = np.max(self.map_points[:, 0])
            min_y = np.min(self.map_points[:, 1])
            max_y = np.max(self.map_points[:, 1])
            ...
            self.zoom_level = max(scale_x, scale_y) * padding
            ...
            self.left = center_x - self.zoomed_width / 2
            self.right = center_x + self.zoomed_width / 2

            self.bottom = center_y - self.zoomed_height / 2
            self.top = center_y + self.zoomed_height / 2
